...
Meteor.methods({
	...
	getAudioVideoHistory({ rid, latest, oldest, inclusive, count = 20, unreads }) {
		check(rid, String);
		...
		records = RocketChat.models.Messages.findAudioVideoVisibleByRoomIdBetweenTimestamps(rid, latest, options).fetch();
