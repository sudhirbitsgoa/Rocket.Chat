...
Meteor.methods({
	forwardMessage(params) {
		...
		const message = RocketChat.models.Messages.findOneById(params.messageId);
		...
		message.ts = new Date();
		delete message._id;
		message.origin = params.messageId;
		...
		return RocketChat.forwardMessage(user, message, room);
	},
});
// Limit a user, who does not have the "bot" role, to sending 5 msgs/second
RocketChat.RateLimiter.limitMethod('forwardMessage', 5, 1000, {
	userId(userId) {
